...
async def test_light_calibrate(hass: HomeAssistant):
    ...
    assert 0.3 == await strategy.calculate(State(entity_id, STATE_ON, {ATTR_BRIGHTNESS: 1}))
    assert 1.25 == await strategy.calculate(State(entity_id, STATE_ON, {ATTR_BRIGHTNESS: 10}))
    assert 2.375 == await strategy.calculate(State(entity_id, STATE_ON, {ATTR_BRIGHTNESS: 30}))
    assert 5.15 == await strategy.calculate(State(entity_id, STATE_ON, {ATTR_BRIGHTNESS: 75}))
    assert 15.3 == await strategy.calculate(State(entity_id, STATE_ON, {ATTR_BRIGHTNESS: 255}))

    # set to some out of bound brightness.
    assert 15.3 == await strategy.calculate(State(entity_id, STATE_ON, {ATTR_BRIGHTNESS: 350}))
...
async def test_error_on_non_number_state(hass: HomeAssistant, caplog: pytest.LogCaptureFixture):
...
async def test_validate_raises_exception_when_min_power_higher_than_max(hass: HomeAssistant):
    with pytest.raises(StrategyConfigurationError):
        await _create_strategy_instance(
            hass,
            create_source_entity("light"),
