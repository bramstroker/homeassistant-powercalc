...
| Apple | Printed Apple hardware model numbers such as `A2374` and `A2825` | Apple regulatory product tables and information guides; then HA Apple TV Device Info and pyatv model mappings | Regional or color-specific `M...` order/SKU codes are not the canonical hardware model. Preserve exact pyatv names as discovery aliases and former directory IDs as legacy IDs. Follow the detailed Apple guidance below. |
...
4. When replacing an established marketing-name or SKU directory with a verified A-number, add the former directory ID to `legacy_ids`. Also add it to `aliases` only when it is a genuine discovery value. Thus an old `HomePod Mini` directory belongs in both lists because pyatv reports that exact value, while an old `MQJ83` directory belongs only in `legacy_ids` when the originating Device Info reported `HomePod (gen 2)` instead.
...
6. When replacing an integration-derived or mistyped directory ID, add the former directory ID to `legacy_ids`. Keep it in `aliases` as well only if it remains a genuine discovery value; a typo such as `PS-S020E` belongs only in `legacy_ids`, while a former `lumi.*` directory normally belongs in both lists when integrations report it.
...
4. When renaming an existing pack-SKU directory to the individual-device code, add the former directory ID to `legacy_ids` so existing installations migrate. Keep it in `aliases` as well when it remains a real or documented Home Assistant discovery value; these fields serve independent migration and discovery purposes.
...
5. When replacing a project-derived or internal directory ID with the verified hardware code, add the former directory ID to `legacy_ids`. Keep it in `aliases` as well when Home Assistant actually reports the same value as the model; the two fields independently preserve existing selections and discovery.
...
7. When renaming a directory after establishing a better canonical identifier, add the former directory ID to `legacy_ids`. Keep it in `aliases` as well only when an integration reports that exact value; migration and discovery are separate concerns.
