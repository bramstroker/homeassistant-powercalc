...
4. Add a conflicting integration-reported code to `aliases` only after confirming that it uniquely identifies the same physical variant. A stale PR value, converter bug, or catalog mapping alone is not a discovery alias.
