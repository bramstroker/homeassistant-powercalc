...
- Set `product_url` only to a stable HTTPS page from the manufacturer or brand owner that identifies the exact model or measured variant. Prefer the canonical URL after redirects. Do not use search results, category pages, or third-party retailer listings; a retailer's page is acceptable only when that retailer owns the product brand. Do not link a current successor or revised product page when its electrical specifications differ from the measured profile. A model-specific archived, support, or technical page is acceptable when the original product page no longer exists and the identity remains unambiguous.
