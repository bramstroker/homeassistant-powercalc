...
- Prefer an exact product page over a family, identification, knowledgebase, or technical page. A family page is acceptable only when it explicitly contains the measured variant, and a model-specific archived, support, knowledgebase, or technical page is acceptable when no product page survives. Do not replace a precise legacy source with a URL that redirects to a category page.
- Do not use search results, category pages, or third-party retailer listings; a retailer's page is acceptable only when that retailer owns the product brand. Do not link a current successor or revised product page when its electrical specifications differ from the measured profile.
