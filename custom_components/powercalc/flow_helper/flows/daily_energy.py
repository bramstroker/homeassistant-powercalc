...
def build_daily_energy_config(user_input: dict[str, Any], schema: vol.Schema) -> dict[str, Any]:
    ...
    user_input = unwrap_choose_selector(dict(user_input), CONF_DAILY_ENERGY_VALUE, daily_energy_choice_key_from_validated_value)
    config: dict[str, Any] = {
        CONF_DAILY_FIXED_ENERGY: {},
    }
...
class DailyEnergyConfigFlow:
    ...
    async def async_step_daily_energy(
        ...
        async def _validate(user_input: dict[str, Any]) -> dict[str, Any]:
            unwrapped = unwrap_choose_selector(dict(user_input), CONF_DAILY_ENERGY_VALUE, daily_energy_choice_key_from_validated_value)
