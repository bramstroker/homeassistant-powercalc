...
from .const import MANUFACTURER_DIRECTORY_MAPPING, MODE_FIXED, MODE_LINEAR
...
class LightModel:
    ...
    def get_directory(self) -> str:
        ...
        return os.path.join(
            ...
            f'data/{manufacturer_directory}/{self._model}'
        )
    ...
    def linear_mode_config(self) -> Optional[dict]:
        if (not self.is_mode_supported(MODE_LINEAR)):
            raise UnsupportedMode(f"Mode linear is not supported by model: {self._model}")
        return self._json_data.get("linear_config")
    
    @property
    def fixed_mode_config(self) -> Optional[dict]:
        if (not self.is_mode_supported(MODE_FIXED)):
            raise UnsupportedMode(f"Mode fixed is not supported by model: {self._model}")
