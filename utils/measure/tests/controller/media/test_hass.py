...
def test_set_volume_converts_percentage_to_home_assistant_level(volume: int, expected: float) -> None:
    client = _mock_client()

    _get_instance(client).set_volume(volume)
...
def test_turn_off_uses_device_power_service() -> None:
    client = _mock_client()

    _get_instance(client).turn_off()

    client.trigger_service.assert_called_once_with("media_player", "turn_off", entity_id="media_player.test")


def test_turn_off_stops_playback_when_power_service_is_unsupported() -> None:
    client = _mock_client()
    client.trigger_service.side_effect = [InternalServerError(500, "Not supported"), None]

    _get_instance(client).turn_off()

    assert client.trigger_service.call_args_list == [
        call("media_player", "turn_off", entity_id="media_player.test"),
        call("media_player", "media_stop", entity_id="media_player.test"),
    ]


def test_turn_off_propagates_connection_failure_without_fallback() -> None:
    client = _mock_client()
    client.trigger_service.side_effect = WebsocketError("Disconnected")
...
def test_turn_off_propagates_stop_playback_failure() -> None:
    client = _mock_client()
    client.trigger_service.side_effect = [
