...
def get_power_tasmota():
    contents = str(urllib.request.urlopen("http://" + DEVICE_IP + "/?m").read())
    contents = contents.split(" W{e}")[0]
    contents = contents.split("{m}")[-1]
    standby_usage = float(contents)
    return standby_usage


def get_power():
    power1 = get_power_tasmota()
    time.sleep(0.5)
    power2 = get_power_tasmota()
    time.sleep(0.5)
    power3 = get_power_tasmota()
...
async def main():
    ...
    async with aiohttp.ClientSession() as aiohttp_session:
        ...
        with open(f"{export_directory}/{color_mode}.csv", "w", newline='') as csv_file:
