...
GitHub Device Flow requires a GitHub OAuth app owned by the Powercalc maintainer or organization. Enable Device Flow on that OAuth app and store its public client ID in the main repository Actions variable `POWERCALC_GITHUB_CLIENT_ID`. The release workflow passes the value into the app image and rejects a release when the variable is missing. A client secret is not used or stored.
