...
async function mount(): Promise<ResultView> {
  const element = new ResultView();
...
function input(element: ResultView, name: string): HTMLInputElement {
  return element.shadowRoot!.querySelector<HTMLInputElement>(`input[name="${name}"]`)!;
}

function submit(element: ResultView): void {
...
describe("profile validation", () => {
  ...
  it("requires a fresh preview after edits and submits the normalized server values", async () => {
    const element = await mount();
    ...
    element.contributionAuth = { connected: true };
    await element.updateComplete;
    input(element, "confirm_contribution").click();
    await element.updateComplete;
    const onSubmit = vi.fn();
    element.addEventListener("contribution-submit", onSubmit);
    element.shadowRoot!.querySelector<HTMLButtonElement>(".contribution-auto button.primary")!.click();
    expect(onSubmit.mock.calls[0]![0].detail.aliases).toEqual(["Alias"]);
    element.contributionError = "GitHub could not create the pull request. Try again.";
    element.contributionErrorField = "product_name";
    await element.updateComplete;
    expect(element.shadowRoot!.querySelector('[role="alert"]')!.textContent).toContain(element.contributionError);
  });
