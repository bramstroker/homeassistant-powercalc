    ...
    return html`
      <section class="panel" aria-labelledby="review-title">
        ...
        <div class="actions">
          ...
          <button class="primary" type="button" @click=${() => this.emit("start")} ?disabled=${this.busy || (this.canOverwrite && !this.overwriteConfirmed)}>${this.busy ? "Starting…" : "Start measurement"}</button>
        </div>
      </section>
    `;
  }
