...
import type { Capabilities, EntityDescriptor, MeasurementRequest, PreflightResponse, SessionEvent, SessionFile, SessionSnapshot } from "../types";
...
export class AppShell extends LitElement {
  ...
  private renderView() {
    ...
    return html`
      <measure-setup-view .capabilities=${this.capabilities} .lights=${this.lights} .powers=${this.powers} .voltages=${this.voltages} .initialRequest=${this.request} .busy=${this.busy} .errorMessage=${this.errorMessage} @preflight=${this.runPreflight}></measure-setup-view>`;
  }
  ...
  private async boot(): Promise<void> {
    ...
    try {
      ...
      [this.capabilities, this.lights, this.powers, this.voltages, this.snapshot] = await Promise.all([
        this.api.getCapabilities(), this.api.getEntities("light"), this.api.getEntities("power"), this.api.getEntities("voltage"), currentPromise,
      ]);
  ...
  private async refreshSnapshot(): Promise<void> {
    try {
      ...
      if (["completed", "failed", "cancelled", "resumable"].includes(this.snapshot.state)) {
        this.eventStream?.close(); this.connectedToEvents = false; this.view = "result"; await this.loadFiles();
      }
    }
  ...
  private stepClass(index: number): string {
    const current = { loading: 0, setup: 0, review: 1, running: 2, result: 3 }[this.view];
