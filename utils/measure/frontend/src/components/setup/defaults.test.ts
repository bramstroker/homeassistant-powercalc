...
describe("developer zero-power recording", () => {
  it.each([false, true])("gates and restores the option with developer mode %s", async (developerMode) => {
    const element = document.createElement("measure-setup-view") as SetupViewElement;
    element.capabilities = { ...capabilities, developer_mode: developerMode };
    element.definitions = [recorderDefinition];
    element.deviceEntities = {
      "*": [{ entity_id: "climate.room", name: "Room", domain: "climate", state: "heat" }],
    };
    ...
    element.initialRequest = {
      ...
      power_meter: { type: "dummy" }, generate_model: false, parameters: capabilities.defaults,
      resume_policy: "new", recorder_purpose: "complex_profile", profile_recipe: "generic",
      tracked_entity_ids: ["climate.room"], continue_on_zero_power: true,
    };
    element.meter = { type: "dummy" };
    document.body.append(element);
    await element.updateComplete;

    const toggle = element.shadowRoot.querySelector<HTMLInputElement>('[name="continue_on_zero_power"]');
    expect(Boolean(toggle)).toBe(developerMode);
    if (toggle) expect(toggle.checked).toBe(true);
    const requests: MeasurementRequest[] = [];
    element.addEventListener("preflight", (event) => requests.push((event as CustomEvent<MeasurementRequest>).detail));
    element.shadowRoot.querySelector("form")!.requestSubmit();
    expect(requests[0]).toMatchObject({ continue_on_zero_power: developerMode });
    if (toggle) {
      toggle.click();
      await element.updateComplete;
      element.shadowRoot.querySelector("form")!.requestSubmit();
      expect(requests[1]).toMatchObject({ continue_on_zero_power: false });
    }
    element.remove();
  });

  it("only shows the opt-in toggle for complex recordings", async () => {
    const element = document.createElement("measure-setup-view") as SetupViewElement;
    ...
    document.body.append(element);
    await element.updateComplete;
    expect(element.shadowRoot.querySelector('[name="continue_on_zero_power"]')).toBeNull();
    selectEntity(entityCombobox(element, "recorder_purpose"), "complex_profile");
    await element.updateComplete;
    const toggle = element.shadowRoot.querySelector<HTMLInputElement>('[name="continue_on_zero_power"]');
    expect(toggle?.checked).toBe(false);
    selectEntity(entityCombobox(element, "recorder_purpose"), "playbook");
    await element.updateComplete;
    expect(element.shadowRoot.querySelector('[name="continue_on_zero_power"]')).toBeNull();
    element.remove();
  });
