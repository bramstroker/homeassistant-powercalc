...
def profile_pull_request_body(
    ...
    *,
    ...
    device_lines = [
        ...
        f"- Model ID: {device.model_id}",
        f"- Product name: {device.product_name}",
    ]
    if measure_device:
        device_lines.append(f"- Measurement device: {measure_device}")
    home_assistant_lines = []
    if measure_type:
        home_assistant_lines.append(f"- Measure type: {measure_type}")
    if device.integration:
        home_assistant_lines.append(f"- Integration: {device.integration}")
    home_assistant = (
        f"{'\n'.join(home_assistant_lines)}\n"
        if home_assistant_lines
