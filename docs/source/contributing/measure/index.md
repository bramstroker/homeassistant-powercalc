...
There are two ways to run a measurement. Both share the same measurement core and produce the same output.

- **[Home Assistant app](home-assistant-app.md) (recommended)** - for Home Assistant OS. Install from the app store and get a guided UI with entity selectors, preflight checks, live progress, and result plots. No tokens or configuration files needed.
- **[CLI - Docker or native Python](setup.md)** - for Home Assistant Container or Core installations, direct device power meters (Shelly, Tuya, Kasa, Tasmota, myStrom), direct Hue control, OCR meters, manual readings, or development.
