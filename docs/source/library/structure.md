...
| `min_version`                     | string           | No | Minimum required Powercalc version for the profile, e.g. `v1.26.0`. Set this when the profile uses an option that older versions ignore  |
| `multi_switch_config`             | object           | No | Configuration for [multi switch](../strategies/multi-switch.md) calculation strategy                                                    |
| `multiply_factor`                 | number or string | No | Default power multiplier, either a number or a custom field placeholder such as `[[panel_count]]`. An explicit sensor setting overrides it. See [Variables](variables.md#scaling-per-unit-measurements) |
