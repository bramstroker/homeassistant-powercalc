...
def generate_supported_model_list():
    ...
    writer.header_list = ["manufacturer", "model id", "name", "calculation modes", "color modes"]
    ...
    with open(os.path.join(project_root, "docs/supported_models.md"), "w") as md_file:
        ...
        rows = []
        for json_path in glob.glob(f"{project_root}/custom_components/powercalc/data/**/model.json", recursive=True):
            with open(json_path) as json_file:
                ...
                rows.append([manufacturer, model, name, ",".join(supported_modes), ",".join(color_modes)])
